...
function serializeSetLog(s: SetLog): SetLog {
  return {
    ...
    ...(s.mediaKey ? { mediaKey: s.mediaKey, mediaType: s.mediaType === 'image' ? 'image' : 'video' } : {}),
  };
}
...
function isLogEntryEmpty(e: LogEntry): boolean {
  ...
  if (Array.isArray(e.sets) && e.sets.some(s => s.weight != null || s.reps != null || s.completed || s.mediaKey)) return false;
  return true;
}
...
function parseSetLog(s: unknown): SetLog {
  ...
  return {
    ...
    ...(typeof o.mediaKey === 'string' && o.mediaKey
      ? { mediaKey: o.mediaKey, mediaType: o.mediaType === 'image' ? 'image' : 'video' }
      : {}),
  };
}
...
export function normalizeRoutineLogsKeys(logs: Record<string, LogEntry>): Record<string, LogEntry> {
  const score = (e: LogEntry) => {
    ...
    if (Array.isArray(e.sets)) {
      n += e.sets.filter((s) => s.weight != null || s.reps != null || s.completed || s.mediaKey).length * 4;
    }
    return n;
  };
