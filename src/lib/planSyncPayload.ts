...
export function buildPlanPatchPayload(r: RoutinePlanPatchInput) {
  ...
  const versionsPayload =
    r.versions?.length && r.versions.length > 0
      ? r.versions.map((v) => {
          /** Cada versión viaja con su ciclo para que el servidor no reescriba las semanas ya pasadas. */
          const vcl = Math.max(1, Math.min(52, v.cycleLength ?? (v.weeks?.length && v.weeks.length < 52 ? v.weeks.length : cl)));
          return {
