...
export function expandRoutineFromApi(raw: {
  ...
  let versions: RoutineVersion[] = (raw.versions || []).map((v) => {
    /** Ciclo propio de la versión: sin esto, cambiar el ciclo de la rutina recortaba mal las semanas antiguas. */
    const vcl = Math.max(1, Math.min(52, v.cycleLength ?? (v.weeks?.length && v.weeks.length < 52 ? v.weeks.length : cycleLength)));
    return {
      ...v,
      cycleLength: vcl,
      weeks: versionWeeksToTemplateOnly(v.weeks, vcl),
    };
  });
