...
export default function App() {
  ...
  useEffect(() => {
    ...
    const loadUserData = async () => {
      try {
        const [routinesRes, tmsRes, historyRes, checkInsRes] = await Promise.all([
          apiGet<any[]>('/api/routines').catch(() => []),
          apiGet<any[]>('/api/training-maxes').catch(() => []),
          apiGet<any[]>('/api/training-maxes/history').catch(() => []),
        ...
        // Usuario nuevo: crear TMs por defecto en DB
        if (!tmsRes?.length) {
          try {
            const created = await Promise.all(
              INITIAL_TMS.map(tm =>
                apiPost<any>('/api/training-maxes', {
                  name: tm.name,
                  value: tm.value,
                  mode: tm.mode,
                  linkedExercise: tm.linkedExercise,
                })
              )
            );
            const mapped: TrainingMax[] = created.map((t: any, i: number) => ({
              id: String(t._id || t.id),
              name: t.name,
              value: Number(t.value),
              mode: t.mode,
              linkedExercise: INITIAL_TMS[i].linkedExercise,
            }));
            setTms(mapped);
            setRms({ bench: mapped[0]?.value ?? 110, squat: mapped[1]?.value ?? 140, deadlift: mapped[2]?.value ?? 190 });
          } catch (e) {
            console.error('[App] Error creando TMs seed:', e);
          }
        }
        ...
        if (tmsRes?.length > 0) {
          ...
          setRms(prev => ({ ...prev, ...rmsFromTms }));
        }
        // Historial
        if (historyRes?.length > 0) {
          setHistory(historyRes.map((h: any) => ({
            date: h.date,
            week: h.week,
            year: h.year,
            rms: h.rms || {},
  ...
  useEffect(() => {
    ...
    const doSync = async () => {
      try {
        const weeks = getWeeksAt(routine, currentWeekOfYear);
        const baseTemplate = routine.baseTemplate?.length ? routine.baseTemplate : deriveBaseTemplateFromWeeks(weeks);
        const weekTypeOverrides = routine.weekTypeOverrides || [];
        const logsObj = Object.fromEntries(
          Object.entries(routine.logs || {}).map(([k, v]) => [
            k,
            { rpe: v?.rpe ?? '', notes: v?.notes ?? '', completed: v?.completed ?? false, sets: v?.sets ?? [] },
          ])
        );
        await apiPut(`/api/routines/${routine.id}`, { weeks, baseTemplate, weekTypeOverrides, logs: logsObj, sameTemplateAllWeeks: routine.sameTemplateAllWeeks, hiddenFromSocial: routine.hiddenFromSocial });
      } catch (e) {
  ...
  }, [routines, activeRoutineId, user?.id, currentWeekOfYear]);
    ...
    updateActiveRoutine((routine) => {
      ...
      if (!isHistoryMode && user) {
        ...
        if (didBump) {
          ...
          const currentDate = new Date().toLocaleDateString('es-ES', { month: 'short' });
          setHistory(prev => {
            const sameWeek = (e: HistoryEntry) => e.year === y && e.week === w;
            const filtered = prev.filter(e => !sameWeek(e));
            const entry = createHistoryEntry(currentDate, newTms, newRms, { week: w, year: y });
          ...
          newTms.filter(t => t.value !== tms.find(ot => ot.id === t.id)?.value).forEach(tm => {
